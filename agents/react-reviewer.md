...
- For detailed React and accessibility patterns, prefer any project-level coding-standards, frontend-patterns, or accessibility guidance available in this environment, chosen based on the code being reviewed.
- Invoke this agent directly (or alongside `typescript-reviewer`) for any code review touching `.tsx`/`.jsx` files.
