...
This skill has no dependency on any other plugin's backend, memory store, or CLI — every operation above runs through the standard `gh` CLI plus standard git. Where a repo also carries branch-protection or auto-merge policy in its own CLAUDE.md (for example, "merge once safe and genuinely reviewed" rules), apply that policy's review bar before running the merge/release commands above.
