...
For the broader routing decision of when a capability should be a rule, a skill, an MCP server, or a plain CLI/API workflow: prefer a rule (CLAUDE.md / project rules) for always-on behavioral constraints, a skill for a reusable multi-step workflow a model should follow, an MCP server when the capability needs live external state or a persistent connection the model can't fake with static instructions, and a plain CLI/API call when it's a one-shot action with no repeated pattern worth packaging.
