...
could break in production, or what must be fixed before a launch. It keeps a
useful production-readiness lens while avoiding unpinned external execution and
...
- During active implementation when the right lens is line-level secure coding;
  use a security-review pass first.
