...
1. **Detect the package manager** by checking, in order: the `CLAUDE_PACKAGE_MANAGER` env var, a `.claude/package-manager.json` file, the `package.json` `packageManager` field, then the lockfile present in the repo root (`package-lock.json` → npm, `pnpm-lock.yaml` → pnpm, `yarn.lock` → yarn, `bun.lockb` / `bun.lock` → bun). If none of these are present, fall back to whichever package manager is actually installed and available on PATH.
